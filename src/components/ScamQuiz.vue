    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
          <div v-if="!started" class="bg-gray-50 border border-gray-200 rounded-xl p-8 text-center">
            ...
            <h3 class="text-xl font-semibold text-gray-900 mb-2">Ready to test your awareness?</h3>
            ...
            <div class="flex flex-wrap justify-center gap-3 mb-6">
              <span v-for="tag in ['Phishing', 'Bank Scams', 'Tech Support', 'Romance Scams', 'Fake Prizes']"
                :key="tag" class="text-xs bg-white border border-gray-200 text-gray-600 px-3 py-1 rounded-full">
                {{ tag }}
              </span>
            </div>
            <button @click="started = true"
              class="bg-green-600 text-white text-sm font-medium px-8 py-3 rounded-lg hover:bg-green-700 transition-colors">
              Start Quiz
            </button>
          </div>
          ...
          <div v-else class="border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div class="bg-gray-50 border-b border-gray-200 px-6 py-4">
              <div class="flex items-center justify-between mb-2">
                ...
                <span class="text-xs text-gray-500">{{ Math.round((sampleQuestion.number / sampleQuestion.total) * 100) }}%</span>
              </div>
            ...
            <div class="px-6 py-6">
              <div class="flex items-start gap-3 mb-6">
                <span class="flex-shrink-0 w-6 h-6 bg-green-100 text-green-700 rounded-full text-xs font-medium flex items-center justify-center">
                  {{ sampleQuestion.number }}
                </span>
                <p class="text-gray-800 text-sm leading-relaxed">{{ sampleQuestion.text }}</p>
              </div>

              <div class="space-y-3">
                <button v-for="(option, i) in sampleQuestion.options" :key="i"
                  class="w-full text-left text-sm border border-gray-200 rounded-lg px-4 py-3 hover:border-green-400 hover:bg-green-50 transition-all text-gray-700">
                  <span class="font-medium text-gray-400 mr-2">{{ String.fromCharCode(65 + i) }}.</span>
                  {{ option }}
                </button>
              </div>
            </div>
            ...
            <div class="border-t border-gray-100 px-6 py-4 bg-gray-50 flex justify-between">
              <button class="text-sm text-gray-500 hover:text-gray-700">Skip</button>
              <button class="text-sm bg-gray-900 text-white px-5 py-2 rounded-lg hover:bg-gray-700 transition-colors">
                Next
              </button>
            </div>
          </div>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-gray-50 border border-gray-200 rounded-xl p-6">
            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-4">Your Score</p>
            ...
            <p class="text-xs text-gray-400">Complete the quiz to see your score</p>
          </div>

          <!-- Tips -->
          <div class="bg-gray-50 border border-gray-200 rounded-xl p-6">
            <p class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-4">Quick Tips</p>
            <ul class="space-y-3">
              <li v-for="tip in [

...
<template>
  <section class="min-h-screen bg-gray-50 py-10">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <h1 class="text-4xl font-bold text-gray-900 text-center mb-2">Is this website safe?</h1>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
        ...
        <div class="lg:col-span-2 space-y-4">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
            <label class="block text-sm font-medium text-gray-700 mb-3">Website address to check:</label>
            <input
              v-model="url"
              @keyup.enter="handleSubmit"
              type="text"
              placeholder="e.g. www.example.com"
              class="w-full border border-gray-200 rounded-lg px-4 py-3 text-base bg-gray-50 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent mb-3"
              :class="error ? 'border-red-300' : ''"
            />
            <button
              @click="handleSubmit"
              :disabled="loading"
              class="w-full bg-green-600 hover:bg-green-700 disabled:opacity-60 disabled:cursor-not-allowed text-white text-base font-medium py-3 rounded-lg transition-colors flex items-center justify-center gap-2"
            >
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="bg-white border border-gray-200 rounded-xl p-4 text-center">
              ...
              <p class="text-xs font-semibold text-gray-800 mb-1">Free to use</p>
              <p class="text-xs text-gray-500">No sign-up needed</p>
            </div>
            <div class="bg-white border border-gray-200 rounded-xl p-4 text-center">
              <div class="flex justify-center mb-2">
                <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p class="text-xs font-semibold text-gray-800 mb-1">Plain English</p>
              <p class="text-xs text-gray-500">No tech jargon</p>
            </div>
            <div class="bg-white border border-gray-200 rounded-xl p-4 text-center">
              <div class="flex justify-center mb-2">
                <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p class="text-xs font-semibold text-gray-800 mb-1">Private</p>
              <p class="text-xs text-gray-500">Nothing stored</p>
            </div>
          </div>

          <div class="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex gap-3 items-start">
        ...
        <div class="lg:col-span-3">
          ...
          <div v-if="result" class="space-y-4">
            ...
            <div class="rounded-2xl border p-5 flex items-start gap-4" :class="vc.banner">
              <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" :class="vc.icon">
                <svg class="w-5 h-5" :class="vc.iconColor" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex-1">
                <p class="font-semibold text-base" :class="vc.title">
                ...
                <p class="text-sm mt-0.5" :class="vc.subtitle">{{ result.hostname }}</p>
                <ul v-if="result.riskFactors.length" class="mt-2 space-y-1">
                  <li v-for="rf in result.riskFactors" :key="rf" class="flex items-start gap-1.5 text-sm text-red-700">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
              <div class="flex items-center justify-between mb-3">
                <p class="text-sm font-semibold text-gray-400 uppercase tracking-wide">Trust Score</p>
                <span class="text-3xl font-bold" :class="scoreStyle[scoreLevel(result.trustScore)].text">
                  {{ result.trustScore }}<span class="text-base font-medium text-gray-400">/100</span>
                </span>
              </div>
              ...
              >
                <svg class="w-3 h-3 transition-transform duration-200" :class="showScoreBreakdown ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
                How is this score calculated?
              </button>
              <div v-if="showScoreBreakdown" class="mt-3 border-t border-gray-100 pt-3">
                <div class="grid grid-cols-12 text-sm font-semibold text-gray-400 uppercase tracking-wide mb-2 px-1">
                ...
                <div class="space-y-2">
                  <div
                    ...
                    :key="cat.label"
                    class="grid grid-cols-12 items-start text-sm px-1 py-1.5 rounded-lg"
                ...
                <div class="grid grid-cols-12 text-sm font-semibold border-t border-gray-200 mt-2 pt-2 px-1">
            ...
            <!-- Check results -->
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
              <p class="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-4">Check results</p>
              <div class="space-y-3">
                <div v-for="check in result.checks" :key="check.label" class="flex items-start gap-3">
                  <StatusIcon :status="check.status" class="mt-0.5" />
                  <div>
                    <p class="text-base font-medium text-gray-800">{{ check.label }}</p>
            ...
            <button @click="reset" class="w-full text-sm text-green-700 font-medium py-2 hover:underline">
          ...
          <div v-else class="h-full min-h-72 bg-white rounded-2xl border border-dashed border-gray-200 flex flex-col items-center justify-center p-10 text-center">
            <svg class="w-10 h-10 text-gray-200 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                d="M21 21l-4.35-4.35M17 11A6 6 0 105 11a6 6 0 0012 0z" />
            </svg>
            <p class="text-sm font-medium text-gray-400">Your results will appear here</p>
            <p class="text-xs text-gray-300 mt-1">Enter a website address on the left to get started</p>
          </div>
